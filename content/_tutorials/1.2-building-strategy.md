...
The next few pages will provide a detailed tutorial to building and running a strategy with AutoTrader. For those short on time,
...
your strategy parameters (using `PARAMETERS`) and which instruments to trade with using this strategy (using `WATCHLIST`). As you can see below, 
the default MACD settings of 12/26/9 are used. The risk-to-reward ratio is also defined by the `RR` key. As mentioned previously, this file is read
by AutoTrader and passed into your strategy when it is instantiated. We will start by backtesting with the EUR/USD, but to make things interesting, 
AutoTrader supports multi-instrument backtesting. This will be showcased later on.
