...
def test_macd_backtest():
    ...
    assert bt_results['no_trades'] == 236
    assert round(bt_results['all_trades']['ending_balance'], 3) == 748.946
    assert bt_results['long_trades']['no_trades'] == 99
    assert bt_results['short_trades']['no_trades'] == 136
